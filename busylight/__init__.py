#!/usr/bin/env python
"""
This class controls a connected Busylight device
"""
import time
import logging
...
class BusyLight():
    ...
    def __init__(
        ...
        self.tone_names = [
            'openoffice', 'funky', 'fairytale', 'kuandotrain',
    ...
    def update_buffer(self):
        ...
        LOGGER.debug('updating buffer: %s', self.buffer)

    def set_color(self, r, g, b, blink_rate=0):
        """
        Set the color
        """
        LOGGER.debug('updating color: %s %s %s %s', r, g, b, blink_rate)
        self.red = r
        self.green = g
        self.blue = b
    ...
    def write(self, buff=None):
        ...
        LOGGER.debug('writing: %s', self.buffer)
        self.device.write(self.buffer)

    def close(self):
        """
        Method to close device connection
        """
        self.device.close()
